...
describe("POST /api/feeds", () => {
  ...
  it("includes userId in the insert values", async () => {
    mockReturning.mockResolvedValue([mockFeed]);
    const event = {
      context: { user: { id: 42 } },
      body: { url: "https://example.com/feed.xml" },
  ...
  it("returns detectedSource in the response even when sourceOverride is used", async () => {
    mockDetectFeedSourceType.mockReturnValue("rss");
    const overrideFeed = { ...mockFeed, source: "podcast", detectedSource: "rss" };
    mockReturning.mockResolvedValue([overrideFeed]);
    const event = {
      context: { user: { id: 1 } },
      body: { url: "https://example.com/feed.xml", sourceOverride: "podcast" },
    ...
    const result = await handler(event);
    expect(result).toMatchObject({ detectedSource: "rss", source: "podcast" });
  });

  it("uses sourceOverride rss when content-detected source is podcast", async () => {
    mockDetectFeedSourceType.mockReturnValue("podcast");
    const rssFeed = { ...mockFeed, source: "rss" };
    mockReturning.mockResolvedValue([rssFeed]);
    const event = {
      context: { user: { id: 1 } },
  ...
  it("does not insert the feed when fetching the feed body times out", async () => {
    const abortError = Object.assign(
      new DOMException("The operation was aborted", "AbortError"),
    );
    mockFetchFeedBody.mockRejectedValue(abortError);
  ...
  it("passes the fetched feed body to detectFeedSourceType", async () => {
    const podcastBody = `<?xml version="1.0"?><rss><channel><item><enclosure url="ep.mp3" type="audio/mpeg"/></item></channel></rss>`;
    mockFetchFeedBody.mockResolvedValue(podcastBody);
    mockLooksLikeValidFeed.mockReturnValue(true);
    mockDetectFeedSourceType.mockReturnValue("podcast");
    const podFeed = { ...mockFeed, source: "podcast", detectedSource: "podcast" };
    mockReturning.mockResolvedValue([podFeed]);
    const event = {
      context: { user: { id: 1 } },
      body: { url: "https://example.com/podcast.xml" },
    };
    await handler(event);
    expect(mockDetectFeedSourceType).toHaveBeenCalledWith(podcastBody);
  });
});
